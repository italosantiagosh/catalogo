(function () {
  ...
  //
  // Existem DUAS caixas iguais na home (uma logo abaixo do hero, outra
  // mais embaixo antes das categorias -- ver conversa: quem chega
  // procurando um santo especifico nao devia precisar rolar a pagina
  // toda pra achar a busca), cada uma com seu proprio input/dropdown
  // dentro do mesmo ".busca-wrap-home" -- por isso inicializar por
  // instancia em vez de getElementById fixo.
  document.querySelectorAll('.busca-wrap-home').forEach((wrap) => {
    const input = wrap.querySelector('.busca-home-input');
    const resultados = wrap.querySelector('.busca-home-resultados');
    if (!input || !resultados) return;
    inicializarBuscaHome(input, resultados);
  });

  function inicializarBuscaHome(input, resultados) {
    let buscaTimer = null;
